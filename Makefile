...
CB_SRC := $(wildcard $(SERVER_ROOT)/$(CB_DIR)/*.c)
CB_OBJ := $(CB_SRC:$(SERVER_ROOT)/$(CB_DIR)/%.c=$(OBJDIR)/server/$(CB_DIR)/%.o)
...
$(OBJDIR):
	...
	@mkdir -p $@/server/$(INTERPRETER_DIR) $@/server/$(ERROR_DIR) $@/server/$(MAIN_DIR) $@/server/$(WRAP_DIR) $@/server/$(CREATE_DIR) $@/server/$(SELECT_DIR) $@/server/$(CHAT_DIR) $@/server/$(CB_DIR)
...
$(CB_OBJ): $(OBJDIR)/server/$(CB_DIR)/%.o : $(SERVER_ROOT)/$(CB_DIR)/%.c
	@echo "\033[0;39m[chat]: \033[0mCompiling source file \033[0;36m"$<"\033[0m!"
	@$(CC) $(FLAGS) -c $< -o $@ -I $(INCDIR) $(LINK) $(LIBFT_H)
	@echo "\033[0;39m[chat]: \033[0mCompiled \033[0;36m"$<"\033[0m successfully!"
