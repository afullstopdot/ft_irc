...
$(OBJDIR):
	...
	@mkdir -p $@/server/$(INTERPRETER_DIR) $@/server/$(ERROR_DIR) $@/server/$(MAIN_DIR) $@/server/$(WRAP_DIR)
	@echo "\033[0;31m[server]: \033[0m\033[0;32mComplete!\033[0m"
	

# server rules

$(BINDIR)/$(SERVER_EXE): $(INTERPRETER_OBJ) $(ERROR_OBJ) $(WRAP_OBJ) $(MAIN_OBJ)
	@make -C libft
	@echo "\033[0;31m[server]: \033[0mLinking object files!"
	@$(LINKER) $(INTERPRETER_OBJ) $(ERROR_OBJ) $(WRAP_OBJ) $(MAIN_OBJ) $(FLAGS) -o $@ $(LIBFT) -I $(INCDIR) $(LIBFT_H)
	@chmod +x $@
...
clean:
	...
	@rm -f $(INTERPRETER_OBJ) $(ERROR_OBJ) $(MAIN_OBJ) $(WRAP_OBJ)
